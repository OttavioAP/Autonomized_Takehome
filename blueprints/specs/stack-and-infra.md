...
- **CI/CD**: **revised from the original MVP plan.** `.github/workflows/deploy.yml` now runs on every push to `main` (plus manual `workflow_dispatch`): a `test` job builds the image and runs pytest inside it, and a `deploy` job — gated on `test` passing via `needs: test` — builds/pushes to ACR and redeploys the Azure Web App. This effectively delivers NMVP-NFR-7 (CI running automated tests, gating deploy) ahead of the original non-MVP schedule, at the user's explicit request. Requires a standing Azure service principal credential (`AZURE_CREDENTIALS`, scoped to just the throwaway resource group via `contributor` role) stored in GitHub Actions secrets — a real tradeoff versus the original "no standing credentials in GitHub" position, accepted for this throwaway ~1-week deployment. See `blueprints/deployment.md` for the full rationale and secret inventory.
